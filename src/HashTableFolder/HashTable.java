...
public class HashTable<K, V> implements Iterable<KeyValue<K, V>> {
    ...
    public int countFinalCollisions() {
        int collisionCount = 0;

        for (LinkedList<KeyValue<K, V>> slot : this.slots) {
            // Если в слоте более одного элемента, это коллизия
            if (slot.size() > 1) {
                collisionCount += slot.size() - 1;  // Добавляем количество элементов, участвующих в коллизии
            }
        }
